...
// trap ClaimRegistrationService's own register()/doRegister() split exists specifically to
// avoid): Spring's scheduler invokes the method through the managed bean reference, which
// already IS the transactional proxy - there's no same-class `this` call bypassing it.
//
