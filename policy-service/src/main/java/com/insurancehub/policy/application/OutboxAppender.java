...
// Not a port/adapter - nothing needs to swap this implementation, unlike PolicyRepository.
// Called once per business write, inside the same @Transactional method as the write itself
// (PolicyCreationService.doCreate/PolicyRenewalService.doRenew), after processedRequests.save
// and before the method returns - so the outbox row lands in the exact same local transaction
// as the business row it describes (docs/adr/0006-outbox-relay.md). Every call site's own
// idempotency check already runs before this is ever reached, so a replayed request can never
